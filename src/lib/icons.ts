/* Icon path data for the imported markup.
   Icons are rendered server-side (see components/Icon.tsx and withIcons below);
   clientInit's injectIcons() stays on only as a fallback for anything added at runtime. */
...
/* Presentation attributes injected onto every icon. Kept byte-identical to what
   injectIcons() used to set at runtime so nothing shifts visually. */
const SVG_ATTRS =
  ` viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor"` +
  ` stroke-width="2" stroke-linecap="round" stroke-linejoin="round"`;

/* Matches the placeholder tags in the imported markup, preserving any class/style
   already on them. `data-l` is deliberately left in place — globals.css sizes icons
   with selectors like `.ic-box svg[data-l]`. */
const PLACEHOLDER = /<svg\b([^>]*?)\bdata-l="([a-z0-9-]+)"([^>]*?)>\s*<\/svg>/g;

/** Expand `<svg data-l="name"></svg>` placeholders in an HTML string into real icons. */
export function withIcons(html: string): string {
  return html.replace(PLACEHOLDER, (whole, pre: string, name: string, post: string) => {
    const paths = ICONS[name];
    if (!paths) return whole;
    return `<svg${pre}data-l="${name}"${post}${SVG_ATTRS}>${paths}</svg>`;
  });
}
